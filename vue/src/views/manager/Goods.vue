  ...
  <div>
    ...
    <div class="table">
      <el-table :data="tableData" stripe class="wrap-header-table" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="content" label="Content" width="120" align="center">
          <template v-slot="scope">
            <el-button size="mini" @click="preview(scope.row.content)">View Details</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="date" label="Date" width="110" show-overflow-tooltip></el-table-column>
        <el-table-column prop="status" label="Review Status" width="130">
          <template v-slot="scope">
            <el-tag type="info" v-if="scope.row.status === 'Pending Review'">Pending</el-tag>
        ...
        <el-table-column prop="userName" label="Seller" width="120" show-overflow-tooltip></el-table-column>
        <el-table-column prop="saleStatus" label="Sale Status" width="120"></el-table-column>
        <el-table-column prop="readCount" label="Views" width="80"></el-table-column>
        <el-table-column label="Actions" align="center" width="260" fixed="right">
          <template v-slot="scope">
    ...
    <el-dialog title="Product Information" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="form" label-width="100px" style="padding-right: 50px" :rules="rules" ref="formRef">
        ...
        <el-form-item label="Release Date" prop="date">
          <el-input v-model="form.date" placeholder="Release date"></el-input>
        </el-form-item>
        <el-form-item label="Status" prop="status">
          <el-select v-model="form.status" style="width: 100%" placeholder="Select status">
            <el-option value="Pending Review" label="Pending Review"></el-option>
        ...
        <el-form-item label="Views" prop="readCount">
          <el-input v-model="form.readCount" placeholder="View count"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    changeStatus(row, status) {
      ...
      this.$confirm(`Are you sure you want to ${action} this product?`, 'Confirm', {type: "warning"}).then(response => {
